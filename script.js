...
const staticWorkItems = staticWorksImages.map((image) => image.closest(".work-item"));
...
const syncWorksBlur = () => {
  ...
  staticWorksImages.forEach((image, index) => {
    if (image.style.filter !== topFilter) image.style.filter = topFilter;
    const item = staticWorkItems[index];
    const overlayFilter = topFilter || "none";
    if (item && item.style.getPropertyValue("--work-hover-overlay-filter") !== overlayFilter) {
      item.style.setProperty("--work-hover-overlay-filter", overlayFilter);
    }
  });
  ...
  blurWorksItems.forEach((item, index) => {
    ...
    const baseWidth = 250;
    const targetWidth = image.naturalWidth > image.naturalHeight ? 600 : 450;
...
const getMoreItemMetrics = (targetIndex, scale) => {
  ...
  const items = blurWorksItems.map((item) => {
    ...
    const targetWidth = isHorizontal ? 600 : 450;
    const aspect =
